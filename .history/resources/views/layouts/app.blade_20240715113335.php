...
<body class="bg-gray-50 font-family-karla">
    ...
    <nav class="left-0 z-10 w-full text-white shadow-lg bg-gradient-to-r from-blue-950 via-blue-950 to-green-800"
        ...
        <div class="container px-6 py-4 mx-auto">
            <div class="flex items-center justify-between">
                ...
                <div class="container flex items-center justify-between px-6 py-8 mx-auto">
                    <button @click="openMobileMenu = !openMobileMenu" class="focus:outline-none sm:hidden">
                    ...
                    <div class="flex flex-col">
                        <a href="{{ route('home') }}" class="flex text-4xl font-bold md:text-normal">
                            <h1>KalibawangMu</h1>
                            {{-- <img src="path/to/logo.png" alt="Brand Logo" class="w-8 h-8 mr-2"> --}}
                        </a>
                        <p class="hidden sm:block">Media Resmi PCM Dekso Kapanewon Kalibawang</p>
                    </div>
                </div>
                <!-- Navigation Links for Desktop -->
                <div class="hidden space-x-6 md:flex items-center">
                    <a href="{{ route('home') }}"
                        class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold shadow-lg transform hover:-translate-y-1 hover:scale-105">Home</a>
                    @foreach ($categories as $category)
                        <a href="{{ route('by-category', $category) }}"
                            class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold shadow-lg transform hover:-translate-y-1 hover:scale-105">{{ $category->title }}</a>
                    @endforeach
                    <a href="{{ route('about-us') }}"
                        class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold shadow-lg transform hover:-translate-y-1 hover:scale-105">About
                        Us</a>
                </div>
        ...
        <div x-show="openMobileMenu" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed inset-y-0 left-0 z-30 w-3/4 h-full p-4 bg-gradient-to-r from-blue-950 via-blue-950 to-green-900 md:hidden">
            <div class="space-y-4">
                <a href="{{ route('home') }}"
