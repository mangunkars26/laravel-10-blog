    ...
    <nav class="fixed top-0 left-0 z-10 w-full text-white shadow-lg bg-gradient-to-r from-blue-900 via-blue-700 to-green-600"
        ...
        <div class="container px-6 py-8 mx-auto fixed-top">
            ...
            <div x-show="openMobileMenu" class="mt-4 md:hidden">
                <a href="{{ route('home') }}"
                    class="block px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-700 hover:text-sm hover:text-bold">Home</a>
                @foreach ($categories as $category)
                    <a href="{{ route('by-category', $category) }}"
                        class="block px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-700 hover:text-sm hover:text-bold">{{ $category->title }}</a>
                @endforeach
                <a href="{{ route('about-us') }}"
                    class="block px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue-700 hover:text-sm hover:text-bold">About
                    Us</a>
            </div>
        </div>
    </nav>
