    ...
    <nav class="fixed top-0 left-0 z-10 w-full text-white shadow-lg bg-gradient-to-r from-blue-900 via-blue-700 to-green-600"
        ...
        <div class="container px-6 py-8 mx-auto fixed-top">
            ...
            <div x-show="openMobileMenu" class="mt-4 md:hidden">
                <a href="{{ route('home') }}"
                    class="relative inline-block px-4 py-2 font-semibold text-blue-100 transition duration-300 rounded-md hover:text-lg hover:font-semibold">

                    <span class="absolute inset-0 transition-opacity bg-white opacity-0 hover:opacity-100">Home</span>
                </a>
                @foreach ($categories as $category)
                    <a href="{{ route('by-category', $category) }}"
                        class="relative inline-block px-4 py-2 font-semibold text-blue-900 transition duration-300 rounded-md hover:text-lg hover:font-semibold">{{ $category->title }}
                @endforeach
                <a href="{{ route('about-us') }}"
