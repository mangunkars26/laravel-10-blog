        ...
        <div class="container px-6 py-8 mx-auto fixed-top">
            <div class="flex items-center justify-between">
                ...
                <div class="hidden space-x-6 md:flex">
                    <a href="{{ route('home') }}"
                        class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue hover:font-semibold">Home</a>
                    @foreach ($categories as $category)
                        <a href="{{ route('by-category', $category) }}"
                            class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue hover:font-semibold">{{ $category->title }}</a>
                    @endforeach
                    <a href="{{ route('about-us') }}"
                        class="px-4 py-2 transition duration-300 rounded-md hover:bg-white hover:text-blue hover:font-semibold">About
                        Us</a>
                </div>
            ...
            <div x-show="openMobileMenu" class="mt-4 md:hidden">
                <a href="{{ route('home') }}"
                    class="block px-4 py-2 font-semibold transition duration-300 rounded-md hover:bg-white hover:text-blue-900 hover:text-lg hover:font-semibold">Home</a>
                @foreach ($categories as $category)
                    <a href="{{ route('by-category', $category) }}"
                        class="block px-4 py-2 font-semibold transition duration-300 rounded-md hover:bg-white hover:text-blue-900 hover:text-lg hover:font-semibold">{{ $category->title }}</a>
                @endforeach
                <a href="{{ route('about-us') }}"
