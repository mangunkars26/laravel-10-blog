...
<body class="bg-gray-200">
    ...
    <div class="w-full py-0 bg-gray-100 text-xs flex items-center justify-between px-4">
        ...
        <div class="flex justify-end items-end">
            ...
            <div class="text-gray-400 hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        ...
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-100 bg-green-850 focus:outline-none transition ease-in-out duration-150">
            ...
            @else
            <a href="{{ route('login') }}"
                class="hover:bg-white hover:text-blue-900 hover:font-semibold rounded py-2 px-4 mx-2">Login</a>
            @endauth
        </div>
    </div>
    <!--Hijri Calendar-->
    ...
    <nav class="navbar-body" x-data="{ openMobileMenu: false, showSearch: false }">
        <div
            class="w-full container bg-green-700 mx-auto flex flex-wrap items-center justify-between text-sm mt-0 px-4 py-2">
            <!-- Logo and Title -->
            <div class="flex items-center">
                <!-- Hamburger Menu for Mobile -->
                <button @click="openMobileMenu = !openMobileMenu"
                    class="focus:outline-none md:hidden text-base text-gray-700 mr-2">
            ...
            <div class="hidden md:flex items-center space-x-4 text-white uppercase ml-4">
                <a href="{{ route('home') }}"
                    class="px-2 py-2 transition transform rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold">Home</a>
                @foreach ($categories as $category)
                <a href="{{ route('by-category', $category) }}"
                    class="px-2 py-2 transition transform rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold">{{ $category->title }}</a>
                @endforeach
                <a href="{{ route('about-us') }}"
                    class="px-2 py-2 transition transform rounded-md hover:bg-white hover:text-blue-900 hover:font-semibold">About
                    Us</a>
            </div>
        </div>
        ...
        <div x-show="showSearch" @click.away="showSearch = false"
            class="absolute inset-x-0 top-16 p-4 flex items-center justify-center md:justify-end" x-cloak>
            <div class="flex items-center w-full max-w-4xl">
                <input type="text" placeholder="Cari..."
                    class="w-full p-2 border text-gray-700 border-gray-300 rounded-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div class="container mx-auto md:hidden">
            ...
            <div x-show="openMobileMenu" @click="openMobileMenu = false"
                ...
                x-transition:leave="transition ease-in duration-300" class="fixed inset-0 z-20 bg-black bg-opacity-50">
            </div>

            <!-- Mobile Navigation Links -->
            <div x-show="openMobileMenu" x-transition:enter="transition ease-out duration-300"
                x-transition:leave="transition ease-in duration-300"
                class="fixed inset-y-0 left-0 z-30 w-1/2 h-full p-2 bg-gray-200">
                <!-- Close Button -->
                ...
                <div class="space-y-4">
                    <a href="{{ route('home') }}"
                        class="block px-4 py-2 font-semibold text-gray-600 transition rounded-md hover:bg-white hover:text-gray-900">Home</a>
                    @foreach ($categories as $category)
                    <a href="{{ route('by-category', $category) }}"
                        class="block px-4 py-2 font-semibold text-gray-600 transition rounded-md hover:bg-white hover:text-gray-900">{{ $category->title }}</a>
                    @endforeach
                    <a href="{{ route('about-us') }}"
                        class="block px-4 py-2 font-semibold text-gray-600 transition rounded-md hover:bg-white hover:text-gray-900">About
                        Us</a>
                </div>
